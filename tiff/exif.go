...
func EncodeWithExif(w io.Writer, img *img48.Img, ex *exif.Exif) error {
	...
	b := img.Bounds()
	iw, ih := b.Dx(), b.Dy()
	// ex.IFDSet.Ensure(0, 0x00fe, exif.TypeUint32).SetInts([]int{0})
	ex.IFDSet.Ensure(0, 0x0100, exif.TypeUint32).SetInts([]int{iw})
	ex.IFDSet.Ensure(0, 0x0101, exif.TypeUint32).SetInts([]int{ih})
	ex.IFDSet.Ensure(0, 0x0102, exif.TypeUint16).SetInts([]int{16, 16, 16})
	...
	ex.IFDSet.Ensure(0, 0x011a, exif.TypeUrational).SetRationals([][2]int{{300, 1}})
	ex.IFDSet.Ensure(0, 0x011b, exif.TypeUrational).SetRationals([][2]int{{300, 1}})
	ex.IFDSet.Ensure(0, 0x011c, exif.TypeUint16).SetInts([]int{1})
